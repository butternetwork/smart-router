...
import { MATIC_TESTNET_INIT_CODE_HASH } from '../../../util/constants';
import { MATIC_TEST_FACTORY_ADDRESS } from '../../../util/addresses';

/*
    Only used for bsc testing network
*/
...
export const computeBscTestPairAddress = ({
  factoryAddress,
  tokenA,
  tokenB,
}: {
...
export class Pair {
  ...
  public static getAddress(tokenA: Token, tokenB: Token): string {
    return computeBscTestPairAddress({
      factoryAddress: MATIC_TEST_FACTORY_ADDRESS,
      tokenA,
      tokenB,
    });
  }
  ...
  public getLiquidityMinted(
    ...
    if (JSBI.equal(totalSupply.quotient, ZERO)) {
      liquidity = JSBI.subtract(
        sqrt(
          JSBI.multiply(tokenAmounts[0]!.quotient, tokenAmounts[1]!.quotient)
        ),
        MINIMUM_LIQUIDITY
      );
    } else {
