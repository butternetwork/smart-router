...
import { ICoinFromPoolDataApi, IReward, IRoute } from "@curvefi/api/lib/interfaces";
...
const _getCoinDecimals = (...coinAddresses: string[] | string[][]): number[] => {
...
const formatNumber = (n: number | string, decimals = 18): string => {
    ...
    const result = !fractional ? integer : integer + "." + fractional.slice(0, decimals)
    if (!result) {
...
export const _getExchangeMultipleArgs = (inputCoinAddress: string, route: IRoute): { _route: string[], _swapParams: number[][], _factorySwapAddresses: string[] } => {
...
const isEth = (address: string): boolean => address.toLowerCase() === ETH_ADDRESS.toLowerCase();
    ...
    for (const route of routes) {
        ...
        if ((_estimatedGasForDifferentRoutesCache[routeKey]?.time || 0) + 3600000 < Date.now()) {
            if (contract.estimateGas) {
                ...
                gasPromise = estimateGas.exchange_multiple(_route, _swapParams, _amount, 0, _factorySwapAddresses, { ...curve.constantOptions, value });
            } else {
...
export const _getBestRouteAndOutput = memoize(
    async (inputCoinAddress: string, inputCoinDecimals: number, outputCoinAddress: string, outputCoinDecimals: number, amount: number | string): Promise<IRoute> => {
