...
async function srcChainRouter(
  tokenIn: Token,
  chainId: number
): Promise<[Map<obj, RouteWithValidQuote[]>, obj[]]> {
  // data of demo
...
async function mapRouter(): Promise<[Map<obj, RouteWithValidQuote[]>, obj[]]> {
  ...
  for (let token of TokenList) {
    ...
    let [total, gas, router] = await findBestRouter(
      ChainId.MAP,
      WBNB_BNB,
      toTargetToken(ChainId.MAP, token),
      amount
    );
    tmp.push({
      index: token,
...
async function targetChainRouter(
  tokenOut: Token,
  chainId: number
): Promise<[Map<obj, RouteWithValidQuote[]>, obj[]]> {
  // data of demo
...
async function crossChainRouter(): Promise<RouteWithValidQuote[][]> {
  ...
  const [nearRouters, nearToken] = await targetChainRouter(
    WNEAR_NEAR,
    1313161554
  );
  ...
  return [
    bscRouters.get(bestRouterOnSrcChain!)!,
    nearRouters.get(bestRouterOnTargetChain!)!,
  ];
}
